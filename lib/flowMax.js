...
var flowMax = function () {
    ...
    var getFollowingFuncs = function (index, _a) {
        var _b = (_a === void 0 ? {} : _a).wrappedDisplayName, wrappedDisplayName = _b === void 0 ? displayName : _b;
        var followingFuncs = funcs.slice(index + 1);
        if (wrappedDisplayName == null) {
            return followingFuncs;
        }
        return __spreadArrays([addDisplayName_1.default(wrappedDisplayName)], followingFuncs);
    };
    var flowLength = funcs.length;
    var wrapExistingDisplayName = function (wrapperStr) {
        return wrapperStr + "(" + (displayName !== null && displayName !== void 0 ? displayName : '') + ")";
    };
    if (flowLength) {
        for (var funcIndex = 0; funcIndex < funcs.length; funcIndex++) {
            ...
            if (getNestedFlowMaxArguments) {
                return flowMax.apply(void 0, __spreadArrays(getPrecedingFuncs(funcIndex), getNestedFlowMaxArguments(), getFollowingFuncs(funcIndex)));
            }
            if (addPropTypes_1.isAddPropTypes(func) ||
                addWrapper_1.isAddWrapper(func) ||
                addWrapperHOC_1.isAddWrapperHOC(func) ||
                branch_avoid_circular_dependency_1.isBranch(func)) {
                var wrappedDisplayName = addPropTypes_1.isAddPropTypes(func)
                ...
                var newFlowMax = flowMax.apply(void 0, __spreadArrays(getPrecedingFuncs(funcIndex), [func(newFollowingFlowMax)]));
                newFlowMax[getArgumentsPropertyName] = function () { return funcs; };
                return newFlowMax;
            }
            var addedDisplayName = addDisplayName_1.isAddDisplayName(func);
            if (addedDisplayName) {
    ...
    var ret = function () {
        ...
        while (index < flowLength) {
            ...
            if (renderNothing_1.isRenderNothing(props)) {
                return null;
            }
            var returnsVal = returns_1.isReturns(props);
            if (returnsVal) {
                return returnsVal[0];
            }
            index++;
